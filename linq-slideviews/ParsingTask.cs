...
public class ParsingTask
{
    ...
    {
        return lines.Skip(1).Select(line => ParseVisitRecord(line, slides));
    }

    public static VisitRecord ParseVisitRecord(string line, IDictionary<int, SlideRecord> slides)
    {
        try
        {
            var data = line.Split(';');
    ...
    public static SlideRecord ParseSlideRecord(string[] data)
    {
        if (data.Length != 3)
            return null;
